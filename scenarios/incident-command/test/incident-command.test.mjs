...
test('SHA-256 timeline verification detects tampering', async () => {
  await withServer({}, async (harness) => {
    ...
    const verification = verifyTimeline(
      tampered,
      incident.id,
      incident.tenantId,
    );
...
test('timeline verification rejects a valid foreign chain in every containing view', async () => {
  await withServer({}, async (harness) => {
    const source = (await createIncident(harness, reporter)).body.incident;
    const otherTenantReporter = {
      tenantId: 'tenant-bravo',
      actorId: 'reporter-two',
      role: 'reporter',
    };
    const target = (
      await createIncident(harness, otherTenantReporter, {
        title: 'Authoritative identity target',
      })
    ).body.incident;

    assert.equal(
      verifyTimeline(source.timeline, source.id, source.tenantId).valid,
      true,
    );
    harness.app.store.state.tenants[target.tenantId].incidents[
      target.id
    ].timeline = structuredClone(source.timeline);

    const direct = verifyTimeline(
      source.timeline,
      target.id,
      target.tenantId,
    );
    ...
    assert.ok(
      direct.issues.some(
        (issue) =>
          issue.code === 'INCIDENT_MISMATCH' &&
          issue.expectedIncidentId === target.id &&
          issue.actualIncidentId === source.id,
      ),
    );
    assert.ok(
      direct.issues.some(
        (issue) =>
          issue.code === 'TENANT_MISMATCH' &&
          issue.expectedTenantId === target.tenantId &&
          issue.actualTenantId === source.tenantId,
      ),
    );
    ...
    const detail = await api(harness, `/api/incidents/${target.id}`, {
      auth: targetAuditor,
    });
    assert.equal(detail.body.incident.auditChain.valid, false);

    const audit = await api(harness, `/api/incidents/${target.id}/audit`, {
      auth: targetAuditor,
    });
    assert.equal(audit.body.auditChain.valid, false);
    assert.ok(
      audit.body.auditChain.issues.some(
        (issue) =>
          issue.code === 'INCIDENT_MISMATCH' &&
          issue.expectedIncidentId === target.id,
      ),
    ...
    const preview = await api(
      harness,
      `/api/incidents/${target.id}/postmortem`,
      { auth: targetAuditor },
    );
    assert.equal(preview.body.auditChain.valid, false);
    assert.match(preview.body.markdown, /Status: INVALID/);

    const download = await api(
      harness,
      `/api/incidents/${target.id}/postmortem?download=1`,
      { auth: targetAuditor },
    );
    assert.equal(download.headers.get('x-audit-chain-valid'), 'false');
    assert.match(download.body, /Status: INVALID/);
  });
});

test('a null timeline tail is invalid across detail, audit, and postmortem views', async () => {
  await withServer({}, async (harness) => {
    const incident = (await createIncident(harness, reporter)).body.incident;
    const storedIncident =
      harness.app.store.state.tenants[incident.tenantId].incidents[incident.id];
    storedIncident.timeline.push(null);
    ...
    assert.equal(direct.valid, false);
    assert.equal(direct.entries, 2);
    assert.equal(direct.headHash, null);
    assert.deepEqual(direct.issues.at(-1), {
      sequence: 2,
      code: 'ENTRY_NOT_OBJECT',
    });

    const generated = buildPostmortemMarkdown(storedIncident);
    assert.match(generated, /Status: INVALID/);
    assert.match(generated, /2:ENTRY_NOT_OBJECT/);

    const detail = await api(harness, `/api/incidents/${incident.id}`, {
      auth: auditor,
    });
    assert.equal(detail.status, 200);
    assert.equal(detail.body.incident.auditChain.valid, false);
    assert.equal(detail.body.incident.auditChain.headHash, null);

    const audit = await api(harness, `/api/incidents/${incident.id}/audit`, {
      auth: auditor,
    });
    assert.equal(audit.status, 200);
    assert.equal(audit.body.auditChain.valid, false);
    assert.equal(audit.body.auditChain.headHash, null);
    assert.ok(
      audit.body.auditChain.issues.some(
        (issue) =>
          issue.sequence === 2 && issue.code === 'ENTRY_NOT_OBJECT',
      ),
    );
    ...
    assert.equal(preview.body.auditChain.valid, false);
    assert.equal(preview.body.auditChain.headHash, null);
    assert.match(preview.body.markdown, /Status: INVALID/);
    assert.match(preview.body.markdown, /2:ENTRY_NOT_OBJECT/);
    ...
    assert.equal(download.headers.get('x-audit-chain-valid'), 'false');
    assert.match(download.body, /Status: INVALID/);
    assert.match(download.body, /2:ENTRY_NOT_OBJECT/);
  });
});
