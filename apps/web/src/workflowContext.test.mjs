...
test('explicit evidence selection stays bound to that run and cannot inherit another review', () => {
  ...
  assert.equal(selectMissionEvidenceRun(entries, reviews, 'another-mission-run'), undefined)
})
...
test('mission UI binds evidence and decisions and pins the target before submitting', async () => {
  ...
  assert.match(card, /rememberEvidenceRun\(pendingRun\.id\)\s+void onVerificationDecision\(pendingRun, approved\)/)
  ...
  assert.match(card, /if \(resumedRunId\) rememberEvidenceRun\(resumedRunId\)/)
