...
// Retained from upstream PR #234; the same real HTTP/child boundary now supplies
// this branch's explicit disposable-fixture opt-in and platform expectation.
...
async function runFixture(platform, {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (request.url === '/api/demo/reset') {
    ...
    } else if (request.url === '/api/corps/fixture/missions/preview') {
      ...
      response.end(JSON.stringify({
        snapshot: { tasks, runs, events: [], missions: tasks.map(task => ({ id: task.mission_id, status: 'ready' })) },
        runners: [{ id: 'fixture-runner', os: reportedOS, connected: true, capabilities: [
          ...['claude-code', 'opencode'].map(name => ({ name, available: platform === 'win32',
            detail: 'spawn=no, stream=no, steer=no, interrupt=no, stop=no, resume=no, usage=no, artifacts=no' })),
          { name: 'workspace-isolation', available: true, source_repository: 'fixture/source',
  ...
  try {
    ...
    child = spawn(process.execPath, [script, platform === 'win32' ? '--expect-windows' : '--expect-unix'], {
      cwd: directory,
      windowsHide: true,
      env: {
        ...
        CRONY_EXTERNAL_ADAPTER_TEST: '1',
        CRONY_EXTERNAL_ADAPTER_OUTPUT: report,
      },
    })
...
test('Unix: accidentally accepting an unsupported provider fails the contract', async () => {
  ...
  assert.notEqual(result.code, 0)
  assert.match(result.stderr, /Unsupported execution must be rejected/)
  assert.equal(result.report, null)
})
...
test('Unix: a refusal with a persisted run is rejected', async () => {
  ...
  assert.notEqual(result.code, 0)
  assert.match(result.stderr, /A rejected launch must not allocate a run/)
  assert.equal(result.report, null)
})
